"""
Simplified RAG Pipeline for PakWheels Used Car Dataset
- TF-IDF vectors (scikit-learn only, no PyTorch dependencies)
...
INDEX_CACHE  = Path(__file__).parent.parent / "data" / "tfidf_index.pkl"
...
class RAGPipeline:
    ...
    def build_index(self, force: bool = False):
        ...
        if INDEX_CACHE.exists() and not force:
            log.info("Loading cached TF-IDF index …")
        ...
        log.info("Generating TF-IDF vectors …")
...
Based on the above data, provide a clear, accurate answer to the user's question."""
        ...
        try:
            ...
            if stats and stats.get("count"):
                return f"Found {stats['count']} matching cars. Average price: {stats.get('avg_price_fmt', 'N/A')}. The search results show various {query_type} options based on your criteria."
            else:
                return f"I found some cars matching your query, but I'm having trouble generating a detailed response right now. Please check the car listings below for details."
